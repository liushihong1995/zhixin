$(function () {
    ...
    $("#getCroppedCanvas").on("click", function () {
        ...
        if(canvas == null){
            ...
            var params = '{"id":"' + id + '","image":"' + imgs + '","type":"'+ "behind" +'"}';
            $.ajax({
                url : "http://localhost:8080/zhixin/boss/uploadCard.action",
                type: "post",
                data:  params,
                processData: false,
                ...
                success: function (data) {
                    if(data==true){
                        $("#image").attr("src","/behindPic/"+ id + ".jpeg");
                        alert("上传成功");
                    }else {
                        alert("上传失败，请再次尝试！");
                    }
...
function getCard(){
    var id = $("#bossIdInput").val().trim();
    $.ajax({
        url: "http://localhost:8080/zhixin/boss/getCard.action",
        type: "post",
        data: id,
        async: false,
        contentType: "application/json;charset=UTF-8",
        success: function (data) {
            var cardBehind = data.cardBehind;
            if(cardBehind==null || cardBehind=="") {
                $("#image").attr("src","../images/cardBehind.png");
            }else {
                $("#image").attr("src","/behindPic/" + cardBehind);
            }
        }
    });
}
